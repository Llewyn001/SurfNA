...
#parser.add_argument('--use_receptor_graph', action='store_true', default=True, help='Device to use for inference, e.g. cuda:0 or cpu')
...
def main_function():
    ...
    if not args.no_model:
        ...
        if args.confidence_model_dir is not None:
            ...
            logger.info('loaded model weight for confidence model')
            #print(confidence_model.use_receptor_graph)  
            state_dict = torch.load(f'{args.confidence_model_dir}/{args.confidence_ckpt}', map_location=torch.device('cpu'))
            missing, _ = confidence_model.load_state_dict(state_dict, strict=False)
            print(missing[:10])                               # 如果出现 rec_* 或 rec_graph_*，说明模型使用了受体图
    ...
    for idx, orig_complex_graph in tqdm(enumerate(test_loader),total = len(test_loader),disable= not accelerator.is_local_main_process):
        ...
        while not success: # keep trying in case of failure (sometimes stochastic)
            try:
                ...
                rec_path = os.path.join(args.data_dir, data_list[0]["name"][0], f'{data_list[0]["name"][0]}_protein.pdb')
                logger.info(f"Receptor file path: {rec_path}")
                ...
                if confidence is not None:
                    ...
                    logger.info(
                        f"{orig_complex_graph['name']} rmsd {np.around(rmsd, 1)[re_order]} centroid distance {np.around(centroid_distance, 1)[re_order]} confidences {np.around(confidence, 4)[re_order]}"
                                )
                ...
                logger.info(f"Failed on {orig_complex_graph['name']}: {str(e)}")
                failures += 1
