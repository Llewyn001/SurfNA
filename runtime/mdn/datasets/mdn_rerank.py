...
class DecoyPDBBind(Dataset):
    ...
    def __init__(self, manifest_path, base_split_path, root, transform=None, cache_path='data/cache',
        ...
        self.base_index = {}
        # Index immutable cache metadata directly. Calling ``get`` here used
        # to deepcopy and sanitize every full graph (and RDKit ligand) merely
        # to read its name. On Dataset V2 that costs many CPU-minutes and
        # hundreds of worker threads before the first scorer batch.
        for idx, graph in enumerate(self.base_dataset.complex_graphs):
            name = graph.name[0] if isinstance(graph.name, (list, tuple)) else graph.name
            key = str(name)
            if key in self.base_index:
                raise ValueError(f'duplicate complex name in base cache: {key}')
            self.base_index[key] = idx
