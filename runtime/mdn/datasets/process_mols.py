...
def obtain_self_dist(res):
    try:
        ...
        # 蛋白质处理
        if resname in allowable_features['possible_amino_acids'][:-17]:
        ...
        # 核酸处理
            ...
            # 选择磷酸骨架关键原子
            ca = res.atoms.select_atoms("name P or name O5'")
            c = res.atoms.select_atoms("name C5'")
            n = res.atoms.select_atoms("name C4'")
            o = res.atoms.select_atoms("name O3'")
        # 计算原子间距离矩阵
        if len(ca) > 0 and len(c) > 0 and len(n) > 0 and len(o) > 0:
            try:
                dists = distances.distance_array(ca.positions, res.atoms.positions)
                if dists is not None and len(dists) > 0:
                    try:
                        ca_o_dist = distances.dist(ca,o)[-1][0]*0.1 if len(distances.dist(ca,o)) > 0 else 0.0
                        o_n_dist = distances.dist(o,n)[-1][0]*0.1 if len(distances.dist(o,n)) > 0 else 0.0
                        n_c_dist = distances.dist(n,c)[-1][0]*0.1 if len(distances.dist(n,c)) > 0 else 0.0
                        return [dists.max()*0.1, dists.min()*0.1, ca_o_dist, o_n_dist, n_c_dist]
                    except Exception:
                        return [dists.max()*0.1, dists.min()*0.1, 0.0, 0.0, 0.0]
                else:
                    return [0, 0, 0, 0, 0]
            except Exception:
                return [0, 0, 0, 0, 0]
...
def obtain_resname(res):
    ...
    if resname in METAL:
        return "M"
    elif resname in ['A', 'U', 'T', 'C', 'G', 'DA', 'DT', 'DC', 'DG', 'DU']:
        return resname
    # 处理特殊核酸命名
    elif resname in ['ADE', 'THY', 'CYT', 'GUA', 'URA']:
        return resname[:1] 
    else:
        return resname
...
allowable_features = {
    ...
    'possible_amino_acids': ['ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU', 'GLY', 'HIS', 'ILE', 'LEU', 'LYS', 'MET',
                             ...
                             'PTR', 'GLV', 'CYT', 'SEP', 'HIZ', 'CYM', 'GLM', 'ASQ', 'TYS', 'CYX', 'GLZ','M', 'A', 'U', 'T', 'C', 'G', 'DA', 'DT', 'DC', 'DG', 'DU',
                             'ADE', 'THY', 'CYT', 'GUA', 'URA' 'misc'],
    # 'possible_atom_type_2': ['C*', 'CA', 'CB', 'CD', 'CE', 'CG', 'CH', 'CZ', 'N*', 'ND', 'NE', 'NH', 'NZ', 'O*', 'OD',
    #                          'OE', 'OG', 'OH', 'OX', 'S*', 'SD', 'SG','P', 'OP1', 'OP2',  'OP3',  'O5\'', 'C5\'', 'C4\'', 'O4\'', 'C3\'', 'O3\'', 'C2\'', 'C1\'', 'N1', 'C2', 'O2', 'N2',   
    #                          'N3',  'C4',  'N4',  'C5',   'C6',  'N6',  'N7', 'C8', 'N9', 'misc'],
...
def extract_receptor_structure(rec, lig, save_file,lm_embedding_chains=None):
    # Rebuild this derived pocket deterministically on every graph construction.
    # Returning a tuple of Nones when a previous smoke left ``*_pure.pdb`` made
    # the same complex non-idempotently disappear on a second audit run.
    ...
    for i, chain in enumerate(rec):
        ...
        for res_idx, residue in enumerate(chain):
            ...
            for atom in residue:
                ...
                if atom.name in {'N', 'N1', 'N9'}:
                    n = list(atom.get_vector())
                if atom.name in {'C', "C1'"}:
...
def initConformer(lig_mol,inference_mode='Screen'):
    ...
    return lig_mol
def get_lig_graph_with_matching(mol_, complex_graph, popsize, maxiter, matching, keep_original, num_conformers, remove_hs, max_seconds=180):
    ...
    if matching:
        ...
        for i in range(num_conformers):
            if time.time() - _t0 > max_seconds:
                raise TimeoutError(f"ligand matching exceeded {max_seconds}s")
            ...
            if rotable_bonds:
                optimize_rotatable_bonds(mol_rdkit, mol, rotable_bonds, popsize=popsize, maxiter=maxiter, max_seconds=max_seconds)
...
def obtain_ca_pos(res):
    ...
    elif obtain_resname(res) in ['A','U','T','C','G','DA','DT','DC','DG','DU']:
        try:
            return res.atoms.select_atoms("name C4'").positions[0]
        except:
...
# =========================
# Nucleic-acid specific features (lightweight)
# These features are stored as `complex_graph['receptor'].nucleic_feat` and DO NOT
# change the existing `complex_graph['receptor'].x` dimension (keeps backward compatibility).
# Feature layout (float32, per-residue):
#   [is_nucleic(1), torsion_7(rad), sugar_pucker_2, bfactor_1(norm), stacking_1]
# Total dim = 12 by default.
# =========================
NUCLEIC_RESNAMES = {'A', 'U', 'T', 'C', 'G', 'DA', 'DT', 'DC', 'DG', 'DU'}

def _safe_first_pos(res, atom_name: str):
    try:
        ag = res.atoms.select_atoms(f"name {atom_name}")
        if ag is None or len(ag) == 0:
            return None
        return ag.positions[0]
    except Exception:
        return None

def _dihedral(p1, p2, p3, p4):
    """Return signed dihedral angle in radians. Input: (3,) numpy arrays."""
    b1 = p2 - p1
    b2 = p3 - p2
    b3 = p4 - p3
    n1 = np.cross(b1, b2)
    n2 = np.cross(b2, b3)
    n1n = n1 / (np.linalg.norm(n1) + 1e-8)
    n2n = n2 / (np.linalg.norm(n2) + 1e-8)
    cosang = np.clip(np.dot(n1n, n2n), -1.0, 1.0)
    ang = np.arccos(cosang)
    sign = np.sign(np.dot(np.cross(n1n, n2n), b2 / (np.linalg.norm(b2) + 1e-8)))
    return float(ang * sign)

def _compute_sugar_pucker(res):
    """Return [phase, amplitude] (rough approximation) from sugar ring atoms."""
    c1 = _safe_first_pos(res, "C1'")
    c2 = _safe_first_pos(res, "C2'")
    c3 = _safe_first_pos(res, "C3'")
    c4 = _safe_first_pos(res, "C4'")
    o4 = _safe_first_pos(res, "O4'")
    if any(v is None for v in [c1, c2, c3, c4, o4]):
    ...
    n1n = n1 / (np.linalg.norm(n1) + 1e-8)
    n2n = n2 / (np.linalg.norm(n2) + 1e-8)
    cos_phase = np.clip(np.dot(n1n, n2n), -1.0, 1.0)
    phase = float(np.arccos(cos_phase))
    amplitude = float(np.linalg.norm(c2 - o4) * 0.1)
    return np.array([phase, amplitude], dtype=np.float32)

def _compute_nucleic_torsions(mdn_rec, idx: int):
    """
    Compute 7 nucleic torsions for residue idx (radians):
      α: O3'(i-1)-P-O5'-C5'
      β: P-O5'-C5'-C4'
      γ: O5'-C5'-C4'-C3'
      δ: C5'-C4'-C3'-O3'
    ...
    Missing atoms => 0.
    """
    tors = np.zeros(7, dtype=np.float32)
    try:
        res = mdn_rec.residues[idx]
        prev_res = mdn_rec.residues[idx - 1] if idx - 1 >= 0 else None
        next_res = mdn_rec.residues[idx + 1] if idx + 1 < len(mdn_rec.residues) else None

        # Common atoms for current residue
        P = _safe_first_pos(res, "P")
        O5 = _safe_first_pos(res, "O5'")
        C5 = _safe_first_pos(res, "C5'")
        C4 = _safe_first_pos(res, "C4'")
        C3 = _safe_first_pos(res, "C3'")
        O3 = _safe_first_pos(res, "O3'")

        # α
        if prev_res is not None:
            O3_prev = _safe_first_pos(prev_res, "O3'")
            if all(v is not None for v in [O3_prev, P, O5, C5]):
                tors[0] = _dihedral(O3_prev, P, O5, C5)
        # β
        if all(v is not None for v in [P, O5, C5, C4]):
            tors[1] = _dihedral(P, O5, C5, C4)
        # γ
        if all(v is not None for v in [O5, C5, C4, C3]):
            tors[2] = _dihedral(O5, C5, C4, C3)
        # δ
        if all(v is not None for v in [C5, C4, C3, O3]):
            tors[3] = _dihedral(C5, C4, C3, O3)
        # ε, ζ require next residue
        if next_res is not None:
            Pn = _safe_first_pos(next_res, "P")
            O5n = _safe_first_pos(next_res, "O5'")
            if all(v is not None for v in [C4, C3, O3, Pn]):
                tors[4] = _dihedral(C4, C3, O3, Pn)
            if all(v is not None for v in [C3, O3, Pn, O5n]):
                tors[5] = _dihedral(C3, O3, Pn, O5n)
        # χ
        O4 = _safe_first_pos(res, "O4'")
        C1 = _safe_first_pos(res, "C1'")
        N9 = _safe_first_pos(res, "N9")
        N1 = _safe_first_pos(res, "N1")
        C4b = _safe_first_pos(res, "C4")
        C2b = _safe_first_pos(res, "C2")
        if all(v is not None for v in [O4, C1]):
            if N9 is not None and C4b is not None:
                tors[6] = _dihedral(O4, C1, N9, C4b)
            elif N1 is not None and C2b is not None:
                tors[6] = _dihedral(O4, C1, N1, C2b)
    except Exception:
        pass
    return tors

def rec_nucleic_extra_featurizer(mdn_rec):
    """
    Return per-residue nucleic extra features as torch.float32 tensor with shape (N_res, 12).
    For protein residues, returns mostly zeros (keeps the same shape).
    """
    num_res = len(mdn_rec.residues)
    if num_res == 0:
        return torch.zeros((0, 12), dtype=torch.float32)

    # Precompute base centers for rough stacking detection (nucleic only)
    base_centers = [None] * num_res
    for i, res in enumerate(mdn_rec.residues):
        resname = obtain_resname(res)
        if resname not in NUCLEIC_RESNAMES:
            continue
        try:
            # Rough base atom set
            base_ag = res.atoms.select_atoms("name N1 N3 N7 N9 C2 C4 C5 C6 C8")
            if base_ag is None or len(base_ag) < 3:
                continue
            base_centers[i] = base_ag.positions.mean(axis=0)
        except Exception:
            continue

    feats = np.zeros((num_res, 12), dtype=np.float32)
    for i, res in enumerate(mdn_rec.residues):
        resname = obtain_resname(res)
        is_nuc = 1.0 if resname in NUCLEIC_RESNAMES else 0.0
        feats[i, 0] = is_nuc

        if is_nuc > 0:
            tors = _compute_nucleic_torsions(mdn_rec, i)  # (7,)
            pucker = _compute_sugar_pucker(res)           # (2,)
            feats[i, 1:8] = tors
            feats[i, 8:10] = pucker

            # B-factor (if present)
            try:
                tf = getattr(res.atoms, "tempfactors", None)
                if tf is not None and len(tf) > 0:
                    feats[i, 10] = float(np.nanmean(tf) * 0.01)
            except Exception:
                pass

            # Stacking (rough): any other base center within [3.0, 4.5] Å
            ci = base_centers[i]
            if ci is not None:
                try:
                    stacked = 0.0
                    for j, cj in enumerate(base_centers):
                        if j == i or cj is None:
                            continue
...
def rec_nucleic_anchor_featurizer(mdn_rec):
    """Return explicit base/sugar/phosphate anchors and their parent residues."""
    positions, roles, parents = [], [], []
    for residue_index, res in enumerate(mdn_rec.residues):
        if obtain_resname(res) not in NUCLEIC_RESNAMES:
            continue
        try:
            base = res.atoms.select_atoms("name N1 N3 N7 N9 C2 C4 C5 C6 C8")
            if base is not None and len(base) >= 3:
                positions.append(base.positions.mean(axis=0)); roles.append(0); parents.append(residue_index)
            sugar = _safe_first_pos(res, "C4'")
            if sugar is not None:
                positions.append(sugar); roles.append(1); parents.append(residue_index)
            phosphate = _safe_first_pos(res, 'P')
            if phosphate is not None:
                positions.append(phosphate); roles.append(2); parents.append(residue_index)
        except Exception:
            continue
    if not positions:
        return (torch.zeros((0, 3), dtype=torch.float32), torch.zeros((0,), dtype=torch.long),
                torch.zeros((0,), dtype=torch.long))
    return (torch.tensor(np.asarray(positions), dtype=torch.float32), torch.tensor(roles, dtype=torch.long),
...
def get_calpha_graph(mdn_rec,c_alpha_coords, n_coords, c_coords, complex_graph, cutoff=20, max_neighbor=None, lm_embeddings=None):
    ...
    # nucleic extra features (optional)
    try:
        complex_graph['receptor'].nucleic_feat = rec_nucleic_extra_featurizer(mdn_rec)
        anchor_pos, anchor_type, anchor_parent_index = rec_nucleic_anchor_featurizer(mdn_rec)
        complex_graph['receptor'].nucleic_anchor_pos = anchor_pos
        complex_graph['receptor'].nucleic_anchor_type = anchor_type
        complex_graph['receptor'].nucleic_anchor_parent_index = anchor_parent_index
    except Exception as e:
        # 静默处理异常，不设置nucleic_feat属性
        pass
    return
...
#     return atom_feats
def rec_atom_featurizer(rec):
    """基于 MDAnalysis Universe 提取原子特征"""
    atom_feats = []
    for atom in rec.atoms:  # 使用 MDAnalysis 的 atoms 属性
        # 提取原子名称和元素类型
        atom_name = atom.name
        element = atom.type.strip().capitalize()  # MDAnalysis 中元素类型存储在 atom.type
        
        # 处理特殊元素（如 CD -> C）
        if element == 'CD':
            element = 'C'
        assert element != '', f"原子 {atom.id} 的元素类型为空"
        ...
        # 提取残基名称
        resname = atom.resname.strip().capitalize()  # 从原子所属残基获取名称
        
        # 构建特征向量
        atom_feat = [
            safe_index(allowable_features['possible_amino_acids'], resname),
            safe_index(allowable_features['possible_atomic_num_list'], atomic_num),
            safe_index(allowable_features['possible_atom_type_2'], (atom_name + '*')[:2]),
            safe_index(allowable_features['possible_atom_type_3'], atom_name)
        ]
...
def get_fullrec_graph(rec, rec_coords, c_alpha_coords, n_coords, c_coords, complex_graph, c_alpha_cutoff=20,
    ...
    # nucleic extra features (optional)
    try:
        complex_graph['receptor'].nucleic_feat = rec_nucleic_extra_featurizer(rec)
        anchor_pos, anchor_type, anchor_parent_index = rec_nucleic_anchor_featurizer(rec)
        complex_graph['receptor'].nucleic_anchor_pos = anchor_pos
        complex_graph['receptor'].nucleic_anchor_type = anchor_type
        complex_graph['receptor'].nucleic_anchor_parent_index = anchor_parent_index
    except Exception as e:
