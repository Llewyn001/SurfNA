...
class PDBBind(Dataset):
    ...
    def get_complex(self, par):
        ...
        if ligand is not None:
            ...
            # get one candidate docking pose and it's RMSD
            ligs, rmsd_list = [read_abs_file_mol(os.path.join(self.pdbbind_dir, name,f'{name}_ligand.sdf'), remove_hs=False, sanitize=True)], extract_rmsd_from_sdf(os.path.join(self.pdbbind_dir, name,f'{name}_ligand.sdf'))
        ...
        for i, (lig, RMSD_) in enumerate(zip(ligs,rmsd_list)):
...
def extract_rmsd_from_sdf(sdf_file):
    scores = []
    ...
    for mol in molecules:
        ...
        for i, line in enumerate(lines):
            ...
            if line.strip().startswith('>  <score>'):
                # 确保下一行存在
                if i + 1 >= len(lines):
                    raise ValueError(f"Missing score value in molecule block: {mol}")
                score_str = lines[i + 1].strip()
                # 转换为浮点数
                try:
                    score = float(score_str)
                except ValueError:
                    raise ValueError(f"Invalid score value '{score_str}' in molecule block: {mol}")
                scores.append(score)
                score_found = True
                break
        if not score_found:
            raise ValueError(f"Score property not found in molecule block: {mol}")
    return scores
