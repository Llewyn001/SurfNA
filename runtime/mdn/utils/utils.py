...
def get_optimizer_and_scheduler(args, model, accelerator,scheduler_mode='min'):
    ...
    # If using adaptive transfer, use different learning rates for different layer types
    if hasattr(args, 'use_adaptive_transfer') and args.use_adaptive_transfer:
        surface_backbone_keys = [
            'surface_node_embedding', 'surface_edge_embedding', 'surface_conv_layers',
            'lig_to_surface_conv_layers', 'surface_to_lig_conv_layers',
            'lig_node_embedding', 'lig_edge_embedding', 'lig_conv_layers', 'cross_edge_embedding'
        ]
        nucleic_layer_keys = [
            'rec_node_embedding', 'rec_edge_embedding', 'rec_conv_layers',
            'residue_to_surface_conv_layers', 'nuc_feat_proj', 'nuc_feat_gate'
        ]
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                ...
                elif any(key in name for key in nucleic_layer_keys):
                    nucleic_params.append(param)
                elif any(key in name for key in surface_backbone_keys):
        ...
        param_groups = []
        if adapter_params:
            # Adapter parameters: use base learning rate
            param_groups.append({'params': adapter_params, 'lr': args.lr, 'weight_decay': args.w_decay})
        if nucleic_params:
            # Nucleic-acid layers: use base learning rate
            param_groups.append({'params': nucleic_params, 'lr': args.lr, 'weight_decay': args.w_decay})
        if surface_params:
            surface_lr_ratio = getattr(args, 'surface_lr_ratio', 0.5)
            # In fully frozen transfer, surface params should already have requires_grad=False.
            # Progressive/unfreeze_layers experiments intentionally make a subset trainable.
            if (not getattr(args, 'freeze_backbone', False)
                    or getattr(args, 'unfreeze_layers', '')
                    or getattr(args, 'progressive_unfreeze', False)):
                param_groups.append({'params': surface_params, 'lr': args.lr * surface_lr_ratio, 'weight_decay': args.w_decay})
        if other_params:
            param_groups.append({'params': other_params, 'lr': args.lr, 'weight_decay': args.w_decay})
...
def get_model(args, device, t_to_sigma, no_parallel=False, model_type='score_model'):
    ...
    if model_type == 'mdn_model':
    ...
    elif model_type in {'surface_score_model', 'surface_pretrain_v2'}:
        ...
        model = model_class(t_to_sigma=t_to_sigma,
                    device=device,
                    no_torsion=args.no_torsion,
                    # use_receptor_graph=args.use_receptor_graph,
                    timestep_emb_func=timestep_emb_func,
                    num_conv_layers=args.num_conv_layers,
                    lig_max_radius=args.max_radius,
                    scale_by_sigma=args.scale_by_sigma,
                    ...
                    lm_embedding_type=lm_embedding_type,
                    args=args,
                    confidence_mode=(model_type == 'surface_pretrain_v2'),
                    confidence_no_batchnorm=(model_type == 'surface_pretrain_v2'),
                   )
    else:
