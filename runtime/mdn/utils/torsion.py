...
def get_transformation_mask(pyg_data):
    ...
    # undireted graph, so edges are duplicated, only keep one torsion per edge
...
def modify_conformer_torsion_angles(pos, edge_index, mask_rotate, torsion_updates, as_numpy=False):
    ...
    for idx_edge, e in enumerate(edge_index.cpu().numpy()):
        ...
        if not np.isfinite(rot_norm) or rot_norm < 1e-8:
            continue
        rot_vec = rot_vec * torsion_updates[idx_edge] / rot_norm # idx_edge!
