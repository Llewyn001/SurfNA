...
# Force-field relaxation is optional and is not required for score training or
# ordinary diffusion sampling.  Import its heavy OpenFF/OpenMM dependencies only
# when that explicit post-processing path is requested.
...
def inferenceFFOptimize(data_list,args,receptor_path,N=40):
    try:
        ...
        raise RuntimeError(
            "Force-field relaxation requires the optional OpenFF/OpenMM environment; "
            "score training and ordinary diffusion inference do not."
        ) from exc
...
def sampling(input_data_list, model, inference_steps, tr_schedule, rot_schedule, tor_schedule, device, t_to_sigma, model_args,
    ...
    for t_idx in range(inference_steps):
        ...
        for complex_graph_batch in loader:
            ...
            # with torch.no_grad():
            #     tr_score, rot_score, tor_score, mask_loss = model(complex_graph_batch) 
            with torch.no_grad():
                # for pretrain
                # tr_score, rot_score, tor_score, mask_loss = model(complex_graph_batch) 
