...
for protein in tqdm(proteins ):
        ...
        if not os.path.exists(target_filename):
            target_filename = os.path.join(args.data_dir,protein,f'{protein}_protein.pdb')
        if not os.path.exists(target_filename):
        ...
        if args.Screen_ligand_library_file is not None:
            ...
            logger.info(f'Using Screen ligands library file: {args.Screen_ligand_library_file}')
            ligand_filename = args.Screen_ligand_library_file
        
        if os.path.exists(ref_ligand_filename):

            pocket = os.path.join(args.surface_out_dir, protein, f'{protein}_protein_8A.pdb')
            surface = os.path.join(args.surface_out_dir, protein, f'{protein}_protein_8A.ply')
