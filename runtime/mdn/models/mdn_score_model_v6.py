...
from utils.training_mdn_improved import mdn_loss_fn_improved, calculate_probability_improved
""""
...
class TensorProductScoreModelV6(torch.nn.Module):
    def __init__(self, args,t_to_sigma, device, timestep_emb_func, in_lig_edge_features=10, in_rec_edge_features = 5,sigma_embed_dim=32, sh_lmax=2,
        ...
        # Keep MDN aligned with the docking model surface schema. SurfNA uses
        # hbond, hphob, charge, si by default, while old SurfDock checkpoints
        # used 3 surface features.
        ...
        if self.mdn_mode:
            ...
            if getattr(self.args, 'mdn_rerank_mode', 'native_mdn') == 'decoy_supervised':
                pose_in_dim = mdn_hidden_dim + 1
                self.pose_confidence = nn.Sequential(
                    nn.Linear(pose_in_dim, 2 * mdn_hidden_dim),
                    nn.BatchNorm1d(2 * mdn_hidden_dim) if not mdn_no_batchnorm else nn.Identity(),
                    nn.ELU(),
                    nn.Dropout(p=mdn_dropout),
                    nn.Linear(2 * mdn_hidden_dim, mdn_hidden_dim),
                    nn.ELU(),
                    nn.Dropout(p=mdn_dropout),
                    nn.Linear(mdn_hidden_dim, 1),
                )
    ...
    def forward(self, data):
        ...
        if self.mdn_mode:
            ...
            sigma = F.elu(self.z_sigma(C)) + getattr(self.args, 'mdn_sigma_min', 1.1)
            mu = F.elu(self.z_mu(C)) + getattr(self.args, 'mdn_mu_min', 1.0)
            dist = compute_euclidean_distances_matrix(h_l_pos, h_t_pos)[C_mask]
            pose_score = None
            if getattr(self.args, 'mdn_rerank_mode', 'native_mdn') == 'decoy_supervised':
                pose_prob = calculate_probablity(
                    pi, sigma, mu, dist.unsqueeze(1).detach(),
                    dist_threhold=self.args.mdn_dist_threshold_test if self.args.mdn_dist_threshold_test is not None else 5.0)
                pose_probx = scatter_add(pose_prob, C_batch, dim=0, dim_size=B)
                lig_pool = scatter_mean(scalar_lig_attr, data['ligand'].batch, dim=0, dim_size=B)
                surf_pool = scatter_mean(scalar_rec_attr, data['surface'].batch, dim=0, dim_size=B)
                pose_input = torch.cat([lig_pool, surf_pool, torch.log1p(pose_probx).unsqueeze(-1)], dim=-1).float()
                pose_score = self.pose_confidence(pose_input).squeeze(-1)
            train_dist_threshold = self.args.mdn_dist_threshold_train if self.args.mdn_dist_threshold_train is not None else 7.0
            if getattr(self.args, 'mdn_loss_mode', 'contact_only') == 'weighted_all':
                mdn_loss_interaction = mdn_loss_fn_improved(
                    pi, sigma, mu, dist.unsqueeze(1).detach(),
                    dist_threshold=train_dist_threshold,
                    weight_near=1.0,
                    weight_far=getattr(self.args, 'mdn_far_weight', 0.05),
                    use_distance_weighting=True,
                )
            else:
                mdn_loss_interaction = mdn_loss_fn(pi, sigma, mu, dist.unsqueeze(1).detach(),dist_threhold=train_dist_threshold)
            ...
            if self.args.ligand_distance_prediction:
                ...
                sigma_ligand = F.elu(self.ligand_z_sigma(C_ligand)) + getattr(self.args, 'mdn_sigma_min', 1.1)
                mu_ligand = F.elu(self.ligand_z_mu(C_ligand)) + getattr(self.args, 'mdn_mu_min', 1.0)
                dist_ligand = compute_euclidean_distances_matrix(h_l_pos, h_l_pos)[C_mask_ligand]
                if getattr(self.args, 'mdn_loss_mode', 'contact_only') == 'weighted_all':
                    mdn_loss_ligand = mdn_loss_fn_improved(
                        pi_ligand, sigma_ligand, mu_ligand, dist_ligand.unsqueeze(1).detach(),
                        dist_threshold=train_dist_threshold,
                        weight_near=1.0,
                        weight_far=getattr(self.args, 'mdn_far_weight', 0.05),
                        use_distance_weighting=True,
                    ...
                    mdn_loss_ligand = mdn_loss_fn(pi_ligand, sigma_ligand, mu_ligand, dist_ligand.unsqueeze(1).detach(),dist_threhold=train_dist_threshold)
            else:
            ...
            if self.training:
                ...
                if getattr(self.args, 'mdn_rerank_mode', 'native_mdn') == 'decoy_supervised':
                    return mdn_loss_interaction , mdn_loss_ligand , atom_types_loss , bond_types_loss , residue_types_loss, pose_score
                return mdn_loss_interaction , mdn_loss_ligand , atom_types_loss , bond_types_loss , residue_types_loss

            else:
                # probx = 0.0
                # for dist in dists_list:
                test_dist_threshold = self.args.mdn_dist_threshold_test if self.args.mdn_dist_threshold_test is not None else 5.0
                score_mode = getattr(self.args, 'mdn_score_mode', 'sum')
                if score_mode in ('soft_distance_sum', 'soft_distance_mean'):
                    ...
                    prob = calculate_probablity(pi, sigma, mu, dist.unsqueeze(1).detach(),dist_threhold=test_dist_threshold)
                if score_mode in ('soft_distance_sum', 'soft_distance_mean'):
                    scale = max(float(getattr(self.args, 'mdn_soft_threshold_scale', 1.0)), 1e-6)
                    prob = prob * torch.sigmoid((test_dist_threshold - dist) / scale)
                if score_mode in ('topk_sum', 'topk_mean'):
                    topk = max(1, int(getattr(self.args, 'mdn_score_topk', 64)))
                    selected_prob = torch.zeros_like(prob)
                    for batch_idx in range(B):
                        mask = C_batch == batch_idx
                        if not torch.any(mask):
                            continue
                        vals = prob[mask]
                        k = min(topk, vals.numel())
                        top_vals, top_idx = torch.topk(vals, k)
                        full_idx = torch.where(mask)[0][top_idx]
                        selected_prob[full_idx] = top_vals
                    prob = selected_prob
                probx = scatter_add(prob,C_batch, dim=0, dim_size=B)
                if score_mode in ('mean', 'topk_mean', 'soft_distance_mean'):
                    denom = scatter_add(torch.ones_like(prob), C_batch, dim=0, dim_size=B).clamp_min(1.0)
                    if score_mode == 'topk_mean':
                        denom = denom.clamp_max(float(max(1, int(getattr(self.args, 'mdn_score_topk', 64)))))
                    probx = probx / denom
                if getattr(self.args, 'mdn_rerank_mode', 'native_mdn') == 'decoy_supervised' and pose_score is not None:
                    probx = pose_score
    ...
    def build_surface_conv_graph(self, data):
        ...
        # Cached Cartesian XYZ components depend on the preprocessing frame and
        # are not scalar features.  E3NN receives direction through edge_sh and
        # distance through edge_length_emb, so zero these three legacy slots
        # while preserving checkpoint tensor shapes.
        invariant_placeholders = torch.zeros_like(
            data['surface','surface_edge','surface'].edge_attr
        )
        edge_attr = torch.cat([invariant_placeholders, edge_length_emb], 1).float()
        edge_sh = o3.spherical_harmonics(self.sh_irreps, edge_vec, normalize=True, normalization='component')
        return node_attr, edge_index, edge_attr, edge_sh
