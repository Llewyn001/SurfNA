...
def main_function():
    ...
    for idx, orig_complex_graph in tqdm(enumerate(test_loader),total = len(test_loader),disable= not accelerator.is_local_main_process):
        ...
        while not success: # keep trying in case of failure (sometimes stochastic)
            try:
                ...
                rec_path = os.path.join(args.data_dir, complex_name, f'{complex_name}_protein.pdb')
                logger.info(f"Receptor file path: {rec_path}")
                if not os.path.exists(rec_path):
                    rec_path = os.path.join(args.data_dir, complex_name, f'{complex_name}_protein_obabel_reduce.pdb')
                ...
                if args.save_docking_result:
                    ...
                    raise ValueError('--decoy_manifest_out requires --save_docking_result so pose_sdf files exist')
                ########################################################################
                ...
                error_text = str(e)
                if (
                    "sampling returned None" in error_text
                    or "CUDA" in error_text
                    or "cuda" in error_text
                    or "CUDACachingAllocator" in error_text
                ):
                    raise
                failures += 1
                sample_count_failed += 1
                if sample_count_failed > 5:
                    logger.info(" Skip by five times Failed on", complex_name, e)
                    success = 1
                else:
                    success = 0
    ...
    if args.decoy_manifest_out is not None:
        if accelerator.is_local_main_process:
            ...
            fieldnames = ['complex_name', 'sample_idx', 'pose_sdf', 'rmsd', 'rmsd_source_method', 'centroid_distance', 'min_cross_distance', 'confidence', 'split_path']
