...
def _align_nucleic_anchor_coordinates(complex_graph):
    """Repair legacy caches whose custom NA anchors were not receptor-centered."""
    if 'receptor' not in getattr(complex_graph, 'node_types', []):
        return
    receptor = complex_graph['receptor']
    required = ('nucleic_anchor_pos', 'nucleic_anchor_parent_index', 'pos')
    if not all(hasattr(receptor, attr) for attr in required):
        return
    ...
    if (not torch.is_tensor(anchor_pos) or anchor_pos.numel() == 0 or
            not torch.is_tensor(parent_index) or not torch.is_tensor(receptor_pos) or
            not torch.is_tensor(original_center)):
        return
    anchor_pos = torch.nan_to_num(anchor_pos.float(), nan=0.0, posinf=0.0, neginf=0.0)
    ...
    if not bool(valid.any()):
        ...
        return
    center = original_center.to(anchor_pos).reshape(-1, 3)[0]
    centered_candidate = anchor_pos - center
    parent_pos = receptor_pos.float()[parent_index[valid]]
    current_error = torch.linalg.vector_norm(anchor_pos[valid] - parent_pos, dim=-1).median()
    centered_error = torch.linalg.vector_norm(centered_candidate[valid] - parent_pos, dim=-1).median()
    # A one-Angstrom margin prevents a second subtraction for already-centered caches.
...
def sanitize_cached_surface_graph(complex_graph):
    ...
    _align_nucleic_anchor_coordinates(complex_graph)
        ...
        for i, lig in enumerate(ligs):
            ...
            try:
                with time_limit(timeout_sec):
                    ...
                    if hasattr(complex_graph['receptor'], 'nucleic_anchor_pos'):
                        complex_graph['receptor'].nucleic_anchor_pos -= protein_center.to(
                            complex_graph['receptor'].nucleic_anchor_pos
                        )
                    if self.all_atoms:
...
def construct_loader(args, t_to_sigma):
    ...
    train_sampler = None
    sampling_weights_path = getattr(args, 'train_sampling_weights', None)
    if sampling_weights_path:
        import csv
        with open(sampling_weights_path, newline='') as handle:
            reader = csv.DictReader(handle, delimiter='\t')
            weight_by_name = {
                str(row['complex_id']): float(row['weight']) for row in reader
            }
        dataset_names = []
        sampling_weights = []
        for graph in train_dataset.complex_graphs:
            raw_name = getattr(graph, 'name', '')
            name = raw_name[0] if isinstance(raw_name, (list, tuple)) else raw_name
            name = str(name)
            dataset_names.append(name)
            if name not in weight_by_name:
                raise ValueError(f'Missing train sampling weight for complex {name}')
            sampling_weights.append(weight_by_name[name])
        extra_names = sorted(set(weight_by_name) - set(dataset_names))
        if extra_names:
            logger.info(
                f'Ignoring {len(extra_names)} sampling-weight rows absent from the graph cache; '
                f'examples={extra_names[:10]}'
            )
        sampling_weights = torch.as_tensor(sampling_weights, dtype=torch.double)
        if not torch.isfinite(sampling_weights).all() or bool((sampling_weights <= 0).any()):
            raise ValueError('All train sampling weights must be finite and positive')
        train_sampler = WeightedRandomSampler(
            sampling_weights, num_samples=len(train_dataset), replacement=True,
            generator=train_generator
        )
        train_shuffle = False
        logger.info(
            f'G2.2 weighted sampling enabled: path={sampling_weights_path} '
            f'n={len(sampling_weights)} mean={sampling_weights.mean():.4f} '
            f'min={sampling_weights.min():.4f} max={sampling_weights.max():.4f}'
        )
    train_loader = loader_class(dataset=train_dataset, batch_size=args.batch_size, num_workers=args.num_dataloader_workers, shuffle=train_shuffle, sampler=train_sampler, pin_memory=args.pin_memory, prefetch_factor=prefetch_factor, drop_last=train_drop_last, generator=train_generator)
