...
def optimize_rotatable_bonds(mol, true_mol, rotable_bonds, probe_id=-1, ref_id=-1, seed=0, popsize=15, maxiter=500,
                             mutation=(0.5, 1), recombination=0.8, max_seconds=180):
    ...
    logger.info(f"Starting optimization with {len(opt.rotable_bonds)} rotatable bonds.")
    t0 = time.time()
    ...
    result = differential_evolution(opt.score_conformation, bounds,
                                    ...
                                    callback=_cb)
    logger.info(f"Optimization completed in {result['nit']} iterations with success: {result['success']}")
    if timed_out['flag']:
        raise TimeoutError(f"optimize_rotatable_bonds timed out after {max_seconds}s")
