...
def parse_train_args():
    ...
    parser.add_argument('--resume_training', action='store_true', default=False,
                        help='Resume model, optimizer, EMA, epoch, scheduler and RNG state from a full checkpoint. '
                             'Without this flag restart_dir remains a transfer-weight initialization.')
    parser.add_argument('--checkpoint_interval', type=int, default=0,
                        help='If positive, save an EMA-only snapshot every N completed epochs in addition to last_model.pt.')
    ...
    parser.add_argument('--val_samples_per_complex', type=int, default=1,
                        help='Number of fixed-seed poses generated per validation complex. '
                             'Checkpoint rmsds_lt2/rmsds_lt5 are computed from the best pose per complex.')
    parser.add_argument('--val_inference_seed', type=int, default=20260824,
                        help='Fixed base seed for validation pose generation. RNG state is restored afterwards.')
    ...
    parser.add_argument('--scheduler_patience', type=int, default=20,
                        help='Number of scheduler.step validation events without improvement before LR decay')
    ...
    parser.add_argument('--precision_patch_tokenizer', action='store_true', default=False,
                        help='Use local top-k/cutoff patch pooling with a null token and diffusion-time gate.')
    parser.add_argument('--patch_cutoff', type=float, default=8.0,
                        help='Local surface cutoff in Angstrom for the precision patch tokenizer.')
    parser.add_argument('--patch_topk', type=int, default=32,
                        help='Maximum local surface vertices per ligand atom for the precision patch tokenizer.')
    parser.add_argument('--patch_time_gate_center', type=float, default=0.5,
                        help='Diffusion time at which the precision patch gate is half active.')
    parser.add_argument('--patch_time_gate_width', type=float, default=0.1,
                        help='Width of the precision patch diffusion-time sigmoid gate.')
    parser.add_argument('--patch_multiscale_cutoffs', type=str, default='',
                        help='Comma-separated G2.1 patch cutoffs in Angstrom, e.g. 4.5,8,12. Empty keeps legacy single-scale.')
    parser.add_argument('--patch_realign_layers', type=str, default='',
                        help='Comma-separated zero-based backbone layers after which to reapply the shared patch tokenizer.')
    ...
    parser.add_argument('--precision_nusurf', action='store_true', default=False,
                        help='Use local-frame orientation bias, local cutoff, and stateful NuSurf refinement.')
    parser.add_argument('--nusurf_cutoff', type=float, default=10.0,
                        help='Surface-anchor cutoff in Angstrom for precision NuSurf attention.')
    parser.add_argument('--g22_directional_nusurf', action='store_true', default=False,
                        help='Add a zero-initialized NA local-frame directional value residual to NuSurf.')
    parser.add_argument('--nusurf_realign_layers', type=str, default='',
                        help='Comma-separated zero-based backbone layers at which NuSurf refinement blocks are applied.')
    parser.add_argument('--g21_validity_aware_nucleic', action='store_true', default=False,
                        help='Expand cached raw NA angles to sin/cos plus inferred validity masks and drop the dead B-factor channel.')
    ...
    parser.add_argument('--g22_rot_low_noise_boost', type=float, default=0.0,
                        help='Extra normalized rotation-score weight near t=0; zero preserves G2.1.')
    parser.add_argument('--g22_tor_low_noise_boost', type=float, default=0.0,
                        help='Extra normalized torsion-score weight near t=0; zero preserves G2.1.')
    parser.add_argument('--g22_low_noise_decay', type=float, default=4.0,
                        help='Decay in 1 + boost*exp(-decay*t) for G2.2 rotation/torsion weighting.')
    ...
    parser.add_argument('--task_aux_pair_weight', type=float, default=1.0,
                        help='Weight of the G2.1 native ligand-atom by surface-vertex pair-contact loss.')
    parser.add_argument('--task_aux_anchor_radius', type=float, default=6.0,
                        help='Radius for the sparse anchor-centroid auxiliary; pair-contact remains defined by task_aux_contact_radius.')
    parser.add_argument('--task_aux_warmup_epochs', type=int, default=0,
                        help='Linearly ramp the overall task auxiliary from zero over this many epochs.')
    parser.add_argument('--task_aux_clearance_weight', type=float, default=0.0,
                        help='Weight of the G2.2 native-calibrated severe-overlap auxiliary.')
    parser.add_argument('--task_aux_clearance_cutoff', type=float, default=1.0,
                        help='Current ligand--receptor atom clearance below which an overlap may be labeled severe.')
    parser.add_argument('--task_aux_clearance_margin', type=float, default=0.2,
                        help='Require current clearance to be this much smaller than native clearance.')
    parser.add_argument('--task_aux_clearance_noise_decay', type=float, default=4.0,
                        help='Low-noise emphasis for the G2.2 clearance auxiliary.')
    parser.add_argument('--train_sampling_weights', type=str, default=None,
                        help='Optional TSV with complex_id and weight columns for replacement sampling.')
