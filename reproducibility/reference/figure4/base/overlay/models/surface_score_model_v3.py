...
class TensorProductScoreModel(torch.nn.Module):
    def __init__(self, t_to_sigma, device, timestep_emb_func, in_lig_edge_features=10,in_rec_edge_features = 5, sigma_embed_dim=32, sh_lmax=2,
        ...
        # Analysis-only intervention used by the frozen transfer-mechanism
        # experiments.  This is deliberately a plain Python attribute rather
        # than a parameter or buffer so that checkpoint keys and hashes stay
        # unchanged.  alpha=1 follows the original arithmetic forward path;
        # alpha=0 disconnects every Surface -> ligand/score residual path.
        self.analysis_surface_gate_alpha = 1.0
    ...
    def _analysis_gate_alpha(self):
        alpha = self.analysis_surface_gate_alpha
        if torch.is_tensor(alpha):
            if alpha.numel() != 1:
                raise ValueError('analysis_surface_gate_alpha must be scalar')
            alpha = float(alpha.detach().cpu().item())
        else:
            alpha = float(alpha)
        if not math.isfinite(alpha) or alpha < 0.0 or alpha > 1.0:
            raise ValueError('analysis_surface_gate_alpha must be finite and in [0, 1]')
        return alpha

    @staticmethod
    def _gate_surface_residual(original, surface_updated, alpha):
        # Explicit endpoint branches preserve the untouched alpha=1 path and
        # make alpha=0 numerically and differentiably independent of Surface.
        if alpha == 1.0:
            return surface_updated
        if alpha == 0.0:
            return original
        return original + alpha * (surface_updated - original)

    @staticmethod
    def _gate_surface_message(surface_message, alpha):
        if alpha == 1.0:
            return surface_message
        if alpha == 0.0:
            return torch.zeros_like(surface_message)
        return alpha * surface_message

    def forward(self, data):
        #logger.info(f'forward data: {data.name}')
        self.last_task_aux_loss = None
        self.last_task_aux_metrics = {}
        analysis_surface_gate_alpha = self._analysis_gate_alpha()
        ...
        if self.ligand_patch_tokenizer is not None:
            ligand_scalar_before_surface_patch = lig_node_attr[:, :self.ns]
            ...
            ligand_scalar = self._gate_surface_residual(
                ligand_scalar_before_surface_patch,
                ligand_scalar,
        ...
        for l in range(len(self.lig_conv_layers)):
            ...
            surface_lig_inter_update = self._gate_surface_message(
                surface_lig_inter_update,
                analysis_surface_gate_alpha,
            )
