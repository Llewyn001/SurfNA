...
def evaluator_rmsd(
    ...
    *,
    ...
    prepared_native: np.ndarray | None = None,
    prepared_mol: Any | None = None,
) -> tuple[float, str]:
    ...
    # Derived geometry is intentionally evaluated in float64.  Parent model
    # predictions, one-step updates, and stored coordinates remain untouched
    # float32 evidence; only squaring/norm/alignment leaves float32 here.
    coords = np.asarray(coords[mask], dtype=np.float64)
    native_full = (
        native_ligand_coordinates(graph)
        if prepared_native is None else prepared_native
    )
    native = np.asarray(native_full[mask], dtype=np.float64)
    try:
        mol = remove_all_hs(graph.mol[0]) if prepared_mol is None else prepared_mol
        value = get_symmetry_rmsd(mol, native, [coords])
        parsed = float(np.asarray(value).reshape(-1)[0])
        if not math.isfinite(parsed) or parsed < 0:
            raise FloatingPointError(f"symmetry RMSD is not finite/nonnegative: {parsed!r}")
        return parsed, "symmetry_aware"
    except Exception:
        value = float(np.sqrt(np.mean(np.sum((coords - native) ** 2, axis=1))))
        if not math.isfinite(value) or value < 0:
            raise FloatingPointError(f"float64 coordinate RMSD fallback failed: {value!r}")
        return value, "coordinate_rmsd_fallback"


def tensor_cosine(prediction: torch.Tensor, target: torch.Tensor) -> float:
    prediction = prediction.detach().double().reshape(-1).cpu()
