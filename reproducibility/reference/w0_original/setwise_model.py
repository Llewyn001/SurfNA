"""Unbounded dual-head scorer: LambdaRank@3 x confidence shrinkage."""
...
BASE_PATH = Path('/root/SurfNA_L2_scorer_mechanism_20260901_autodl_v1/code/models.py')
BASE_SHA = '6d58a476451d1c7bc030bcce4ff362f204f15275533cba27559accf463432138'
ARMS = ('W0', 'W1', 'W2', 'W3')
LAMBDA_ARMS = ('W1', 'W3')
SHRINKAGE_ARMS = ('W2', 'W3')


def base_api():
    if hashlib.sha256(BASE_PATH.read_bytes()).hexdigest() != BASE_SHA:
        raise RuntimeError('Frozen base model dependency changed')
    spec = importlib.util.spec_from_file_location('_setwise_frozen_models', BASE_PATH)
...
def make_model(arm, mean, std, score_scale):
    if arm not in ARMS:
        raise ValueError('Unknown setwise arm')
    return TopObjectiveScorer(mean, std, score_scale, auxiliary=True)
