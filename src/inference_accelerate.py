"""SurfNA virtual-screening inference entry point."""
...
parser.add_argument('--keep_input_pose', action='store_true', default=False, help='Keep input ligand coordinates instead of regenerating RDKit conformers')
...
def main_function():
    ...
    if args.tail_index == -1:
        df = df_full[args.head_index:]
    else:
        df = df_full[args.head_index:args.tail_index]
    ...
    logger.info(f'Extracted paths: protein_paths={len(protein_paths)}, pocket_paths={len(pocket_paths)}, ligands_paths={len(ligands_paths)}')
    ...
    all_molecules = 0
    logger.info(f'Starting loop with {len(pocket_paths)} complexes')
    ...
    for pocket_path,ligands_path,ref_ligand,surface_path,protein_path,pocket_center in pbar:
        logger.info(f'Processing complex: pocket_path={pocket_path}, ligands_path={ligands_path}')
        ...
        try:
            ...
            logger.info(f'Creating ScreenDataset: pocket_path={pocket_path}, ligands_path={ligands_path}, keep_input_pose={args.keep_input_pose}')
            try:
                graph_args = confidence_args if confidence_args is not None else score_model_args
                test_dataset = ScreenDataset(pocket_path,ligands_path,ref_ligand,surface_path,pocket_center,transform=None,
                                    receptor_radius=graph_args.receptor_radius,
                                    cache_path=None, split_path=None,
                                    remove_hs=graph_args.remove_hs, max_lig_size=None,
                                    c_alpha_max_neighbors=graph_args.c_alpha_max_neighbors,
                                    matching= False, keep_original=True,
                                    popsize=graph_args.matching_popsize,
                                    maxiter=graph_args.matching_maxiter,
                                    all_atoms=graph_args.all_atoms,
                                    atom_radius=graph_args.atom_radius,
                                    atom_max_neighbors=graph_args.atom_max_neighbors,
                                    esm_embeddings = None,
                                    ...
                                    max_surface_vertices=getattr(graph_args, 'max_surface_vertices', 0))
                test_sample_num = len(test_dataset)
                logger.info(f'ScreenDataset created successfully, test_sample_num={test_sample_num}')
            except Exception as e:
                logger.error(f'Error creating ScreenDataset: {e}')
                import traceback
                        ...
                        try:
                            ...
                            if not args.no_model:
                                ...
                                if confidence is None:
                                    confidence = np.zeros(len(data_list), dtype=float)
                                else:
    ...
    if accelerator.is_local_main_process:
        if all_molecules > 0:
            ...
            logger.error('No molecules were processed!')
