...
class PDBBind(Dataset):
    def __init__(self, root, transform=None, cache_path='data/cache', split_path='data/', limit_complexes=0,
        ...
        #     filter_names = [i.strip() for i in open('data_filter_ligpre').readlines()]
            ...
            try:
                with time_limit(timeout_sec):
                    ...
                    if self.surface_path is not None:
                        ...
                        coord_names = ['x', 'y', 'z']
                        surface_feature_names = ['hbond', 'hphob', 'charge', 'si']
                        missing = [name for name in coord_names + surface_feature_names if name not in vertex_properties]
                        if missing:
...
def construct_loader(args, t_to_sigma):
    ...
    train_loader = loader_class(dataset=train_dataset, batch_size=args.batch_size, num_workers=args.num_dataloader_workers, shuffle=train_shuffle, pin_memory=args.pin_memory, prefetch_factor=prefetch_factor, drop_last=train_drop_last)
    val_loader = loader_class(dataset=val_dataset, batch_size=args.batch_size, num_workers=args.num_dataloader_workers, shuffle=False, pin_memory=args.pin_memory, prefetch_factor=prefetch_factor)
