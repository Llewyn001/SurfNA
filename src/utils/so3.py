...
_default_precomputed = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "precomputed_arrays"))
tmp_path = os.environ.get('precomputed_arrays', _default_precomputed)
if os.path.exists(os.path.join(tmp_path,'.so3_omegas_array2.npy')):
    _omegas_array = np.load(os.path.join(tmp_path,'.so3_omegas_array2.npy'))
    _cdf_vals = np.load(os.path.join(tmp_path,'.so3_cdf_vals2.npy'))
    _score_norms = np.load(os.path.join(tmp_path,'.so3_score_norms2.npy'))
    _exp_score_norms = np.load(os.path.join(tmp_path,'.so3_exp_score_norms2.npy'))
else:
    ...
    np.save(os.path.join(tmp_path,'.so3_exp_score_norms2.npy'), _exp_score_norms)
...
def sample_vec(eps):
    # sample uniform on sphere over SO(3)
    x = np.random.randn(3)
    x /= np.linalg.norm(x)
    return x * sample(eps)
