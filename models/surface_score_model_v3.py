...
class TensorProductScoreModel(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        # build surface graph
        surface_node_attr,surface_edge_index, surface_edge_attr, surface_edge_sh , masked_input, mask = self.build_surface_conv_graph(data,mode=self.confidence_mode) # surface_node_attr N x 3
    ...
    def build_surface_conv_graph(self, data, mode):
        ...
        # For pretrain
        mask = torch.bernoulli(torch.full_like(data['surface'].x, 0.2)).bool()  # 伯努利采样
        masked_input = data['surface'].x.clone()
        data['surface'].x[~mask] = 0
        #######
