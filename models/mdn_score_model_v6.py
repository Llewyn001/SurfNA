...
class TensorProductScoreModelV6(torch.nn.Module):
    def __init__(self, args,t_to_sigma, device, timestep_emb_func, in_lig_edge_features=10, in_rec_edge_features = 5,sigma_embed_dim=32, sh_lmax=2,
                 ...
                 dynamic_max_cross=False, dropout=0.0, lm_embedding_type=None, mdn_mode=True, mode='full',
        ...
        assert mode in ['full', 'surface_pretrain']
        ...
        self.surface_node_embedding = AtomEncoder(emb_dim=ns, feature_dims=[[],3], sigma_embed_dim=sigma_embed_dim)
        ...
        if self.mode == 'surface_pretrain':
            self._frozen_layers = []
            for m in [ self.rec_node_embedding,
                       self.rec_edge_embedding,
                       self.residue_to_surface_conv_layers,
                       self.rec_conv_layers ]:
                for p in m.parameters(): p.requires_grad = False
                self._frozen_layers.append(m)
             # 1. 不会算 loss
            self.skip_residue_type_loss = True
            # 2. 冻结其参数，避免梯度更新
            if hasattr(self, 'residue_types'):
                for p in self.residue_types.parameters():
    ...
    def forward(self, data):
        ...
        if self.mdn_mode:
            scalar_lig_attr = lig_node_attr[:, :self.ns] if self.num_conv_layers < 3 \
                          else torch.cat([lig_node_attr[:, :self.ns], lig_node_attr[:, -self.ns:]], dim=1)
            scalar_surf_attr = surface_node_attr[:, :self.ns] if self.num_conv_layers < 3 \
                           else torch.cat([surface_node_attr[:, :self.ns], surface_node_attr[:, -self.ns:]], dim=1)
            scalar_rec_attr = scalar_surf_attr    
            h_l_x, l_mask = to_dense_batch(scalar_lig_attr, data['ligand'].batch, fill_value=0)
            h_t_x, t_mask = to_dense_batch(scalar_surf_attr, data['surface'].batch, fill_value=0)
            ...
            if self.args.residue_type_prediction and not getattr(self, 'skip_residue_type_loss', False):
            # residue types 38
                ...
                residue_types_loss = torch.tensor(0., device=self.device)
            if self.training:
